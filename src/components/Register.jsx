...
const Register = ({ onRegistered }) => {
  ...
  return (
    <section className="w-full max-w-md mx-auto bg-white/10 rounded-xl p-8 shadow-lg mt-8">
      <h2 className="text-3xl font-bold text-indigo-200 mb-6 flex items-center gap-2"><FaUser className="text-indigo-400" /> Register</h2>
      <form onSubmit={handleSubmit} className="space-y-4 text-left">
        ...
        <div className="flex flex-col">
          ...
          <input type="email" name="email" value={form.email} onChange={handleChange} required className="rounded-lg px-4 py-2 mt-1 bg-white/80 text-indigo-900" placeholder="Enter email" />
        </div>
        <div className="flex flex-col">
        ...
        {success && <div className="mt-4 p-4 rounded-lg bg-green-200 text-green-800 font-semibold">Registration successful!</div>}
